...
//TODO: fix follow button only registering once
// function createFollowBtn(topicMain) {
//     let followBtn = topicMain.querySelector(".topicFollowBtn");
...
function setTopicMain(topicName, desc, followCount, topicImg) {
    //let topicMain = document.getElementById("topicMain");
    ...
    //createFollowBtn(topicMain);
    topicNameEle.innerHTML=`${topicName}`;
    followCountEle.innerHTML=`${followCount} Followers`
    descEle.innerHTML=`${desc}`;
    
    topicImgEle.src = `${topicImg}`;
}

document.getElementById("topicFollowBtn").addEventListener("click", function () {
    let followedBtn = document.getElementById("topicFollowBtn");
    if(isFollowed){
        followedBtn.innerHTML=`Unfollow Topic`;
    } else {
        followedBtn.innerHTML=`Follow Topic`;
    };
});
...
topicName = "Hololive";
followCount = 1212903482;
topicDesc = "Hololive Production (Japanese: ホロライブプロダクション) is a virtual YouTuber agency owned by Japanese tech entertainment company Cover Corporation. In addition to acting as a multi-channel network, Hololive Production also handles licensing, merchandising, music production and concert organization. As of November 2024, the agency manages over 90 VTubers in three target languages (Japanese, Indonesian and English), totaling over 80 million subscribers, including several of the most subscribed VTubers on YouTube and some of the most watched female streamers in the world."
topicImg = "../assets/foob.jpg"
isFollowed = true;

setTopicMain(topicName, topicDesc, followCount, topicImg)
